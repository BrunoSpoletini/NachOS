...
Thread::Thread(const char *threadName, bool joinableParam, unsigned priorityParam)
{
    name     = threadName;
    stackTop = nullptr;
    stack    = nullptr;
    status   = JUST_CREATED;
    joinable = joinableParam;
    if (joinable)
        channel = new Channel("Channel Thread");
...
void
Thread::SetPriority(unsigned priorityParam)
{
    priority = priorityParam;
}
...
bool
Thread::FileExists(int fid) {
    return files->HasKey(fid - 2);
}
